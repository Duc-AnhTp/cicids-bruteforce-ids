% ==============================================================================
% reports/tables/manual/tab_label_mapping.tex
% ==============================================================================
\begin{table}[htbp]
\centering
\caption{Quy tắc ánh xạ nhãn bài toán phân loại nhị phân phát hiện tấn công dò quét mật khẩu.}
\label{tab:label_mapping}
\adjustbox{max width=\textwidth}{%
\begin{tabular}{lclp{7.5cm}}
\toprule
\textbf{Nhãn gốc trong dữ liệu} & \textbf{Nhãn $y$} & \textbf{Xử lý bài toán} & \textbf{Mô tả bản chất an ninh mạng} \\
\midrule
\BENIGN & 0 & \Normal & Lưu lượng mạng của các tác vụ và giao dịch hợp lệ của người dùng nội bộ. \\
\FTP & 1 & \Attack & Hành vi dò quét thử mật khẩu liên tục đối với dịch vụ FTP (Cổng đích 21). \\
\SSH & 1 & \Attack & Hành vi dò quét thử mật khẩu tự động đối với dịch vụ SSH (Cổng đích 22). \\
\bottomrule
\end{tabular}%
}
\end{table}
