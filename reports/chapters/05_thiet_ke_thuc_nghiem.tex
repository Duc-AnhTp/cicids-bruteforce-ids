...
\begin{itemize}
    ...
    \item \textbf{Nền tảng phần mềm}: Python 3.12, \texttt{numpy==1.26.4}, \texttt{pandas==2.2.2}, \texttt{scikit-learn==1.6.1}, \texttt{xgboost==3.0.5}, \texttt{shap==0.48.0}, \texttt{matplotlib==3.9.2}.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Đối với Decision Tree và Random Forest}: Sử dụng tham số \texttt{class\_weight='balanced'}:
    \begin{equation}
...
Quá trình tinh chỉnh siêu tham số được thực hiện thông qua tìm kiếm lưới (Grid Search) hoặc tìm kiếm ngẫu nhiên có kiểm soát (Randomized Search) trên không gian các tham số quan trọng nhất của từng thuật toán:

\begin{enumerate}
    \item \textbf{Cây Quyết Định (Decision Tree)}:
    \begin{itemize}
        \item Tiêu chuẩn phân tách (\texttt{criterion}): \texttt{['gini', 'entropy']}
        \item Độ sâu tối đa (\texttt{max\_depth}): \texttt{[4, 6, 8, 12, None]}
        \item Số mẫu tối thiểu tại nút lá (\texttt{min\_samples\_leaf}): \texttt{[5, 10, 20]}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Tốc độ học (\texttt{learning\_rate}): \texttt{[0.05, 0.08, 0.1]}
...
Đây là giao thức đánh giá trung tâm của bài tập lớn được triển khai trong \texttt{scripts/run\_remaining.py} và lưu trữ tại \texttt{artifacts/week3\_week4/test\_comparison.csv}:
\begin{itemize}
