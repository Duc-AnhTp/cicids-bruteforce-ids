...
Theo chuẩn RFC 7011 (IPFIX) \cite{rfc7011}, một luồng mạng (Flow) được định nghĩa là một tập hợp các gói tin (packets) hoặc khung truyền (frames) đi qua một điểm quan sát (Observation Point) trong một khoảng thời gian nhất định và chia sẻ chung một bộ thuộc tính nhận dạng đặc trưng (Flow Keys). Trong thực tế và trong bối cảnh bài tập lớn này, bộ 5 thuộc tính truyền thống (Traditional 5-Tuple) là cách nhận dạng flow phổ biến được sử dụng:
\begin{equation}
...
Công cụ CICFlowMeter \cite{sharafaldin2018generating} thực hiện tổng hợp các gói tin thuộc cùng một 5-Tuple theo cả hai hướng truyền thông (Forward - từ client đến server, và Backward - từ server về client) để tạo thành một luồng hai chiều (Bi-directional Flow). Một flow được coi là kết thúc khi xảy ra một trong các điều kiện sau:
\begin{enumerate}
    \item Xuất hiện cờ điều khiển đóng kết nối TCP (\texttt{FIN} hoặc \texttt{RST}).
    \item Khoảng thời gian không có gói tin mới vượt quá ngưỡng chờ không hoạt động (Inactivity Timeout, mặc định 15 đến 60 giây).
...
\subsection{Phân loại Flow đã hoàn tất và NIDS thời gian thực}
Một điểm cốt lõi cần làm rõ về mặt lý thuyết: vì toàn bộ các đặc trưng thống kê nói trên chỉ được tính toán đầy đủ \textbf{sau khi flow đã kết thúc hoàn toàn}, các mô hình học máy được xây dựng trên dữ liệu này hoạt động ở cơ chế \textbf{Phân loại Flow đã hoàn tất (Completed-Flow Classification)}.
...
Cây quyết định phân loại (Decision Tree \cite{quinlan1986induction}) phân chia không gian đặc trưng thành các vùng siêu hình hộp rời rạc thông qua một tập hợp các quy tắc rẽ nhánh dạng bậc thang ($x_j \le \theta$).
...
\end{equation}
trong đó $p_k$ là tỷ lệ mẫu thuộc lớp $k$ tại nút đang xét. Ưu điểm nổi bật của Decision Tree là tính minh bạch và khả năng trực quan hóa cấu trúc rẽ nhánh, giúp giải thích logic phân loại. Tuy nhiên, Decision Tree đơn lẻ có phương sai cao và dễ bị quá khớp (overfitting).

\subsection{Rừng ngẫu nhiên (Random Forest)}
Random Forest \cite{breiman2001random} là phương pháp học kết hợp dựa trên kỹ thuật đóng bao (bagging). Mô hình xây dựng một tập hợp gồm $B$ cây quyết định độc lập:
\begin{equation}
...
Hàm mục tiêu được tối thiểu hóa tại bước $t$ bao gồm hàm mất mát khả vi và số hạng chính quy hóa (regularization term) để kiểm soát độ phức tạp của cây:
\begin{equation}
...
\end{equation}
có thể gây hiểu nhầm nghiêm trọng nếu được sử dụng độc lập trên dữ liệu mất cân bằng. Giả sử tập kiểm thử có $97.4\%$ mẫu bình thường và $2.6\%$ mẫu tấn công (như tập Test CICIDS2017 Tuesday). Một bộ phân loại tầm thường luôn dự đoán mọi flow là bình thường ($TP=0, FP=0$) sẽ đạt ngay \textbf{Accuracy = 97.4\%} mặc dù không phát hiện được bất kỳ cuộc tấn công nào (\textbf{Recall = 0.0\%}). Do đó, F1-Score, đường cong Precision-Recall và Average Precision là các chỉ số đánh giá cần thiết.
...
Theo đặc tả của scikit-learn \cite{scikit_learn_ap}, chỉ số \textbf{Average Precision (AP)} được tính bằng giá trị trung bình có trọng số của Precision theo mức tăng Recall giữa các ngưỡng:
\begin{equation}
    \text{AP} = \sum_{n} (R_n - R_{n-1}) P_n
\end{equation}
trong đó $P_n$ và $R_n$ là Precision và Recall tại ngưỡng thứ $n$. AP đánh giá chất lượng xếp hạng (ranking) các mẫu dương trên dải ngưỡng phân loại; AP không đồng nhất hoàn toàn với diện tích tích phân hình thang thô của PR-AUC và không đo lường độ chuẩn xác xác suất (probability calibration).
