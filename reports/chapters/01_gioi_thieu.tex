...
\begin{itemize}
    ...
    \item[\textbf{RQ2:}] \textit{(Mức độ ảnh hưởng của phép chia ngẫu nhiên)} Phép chia ngẫu nhiên tạo ra sự khác biệt hiệu năng (độ lệch đánh giá quá lạc quan) ở mức độ định lượng bao nhiêu so với phép chia phân tách theo thứ tự thời gian?
...
\begin{enumerate}
    ...
    \item \textbf{Phân tích nghiên cứu tình huống MLOps thực tế về cờ refit:} Làm rõ trường hợp pipeline chủ động gộp Train và Validation trước khi truyền vào \texttt{RandomizedSearchCV}\allowbreak\texttt{.fit()} kết hợp với cờ \texttt{refit=True} mặc định của scikit-learn, khiến mô hình được huấn luyện lại trên toàn bộ tập gộp (chứa \DataValidationSSH\ mẫu SSH từ Validation), dẫn đến việc điểm Test F1 tăng từ \XGBTestFOne\ lên \XGBRefitTestFOne.
