...
\begin{enumerate}
    ...
    \item \textbf{Gộp nhãn nhị phân} (\Normal\ so với \Attack): Việc gom chung \FTP\ và \SSH\ vào nhãn $y=1$ có thể che giấu những đặc thù riêng của từng giao thức. Tuy nhiên, việc theo dõi chỉ số phụ Subtype Recall đã giúp khắc phục triệt để hạn chế này.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Giới hạn trong phạm vi ngày Tuesday}: Bài tập lớn tập trung nghiên cứu chuyên sâu vào hai biến thể tấn công brute force (FTP và SSH) trong ngày thứ Ba. Kết luận về sự sụp đổ của Zero-shot Transfer cần được tiếp tục kiểm chứng trên các dạng tấn công khác như DoS/DDoS (ngày Wednesday) hay Web Attacks (ngày Thursday).
\end{enumerate}
